...
from core.registries import device_type_registry, register_function
...
def create_iot_function(device_name, method_name, method_info):
    ...
    async def iot_control_function(context, response_success=None, response_failure=None, **params):
        try:
            # 设置默认响应消息
            ...
            # 发送控制命令
            await send_iot_conn(context, device_name, method_name, params)
            # 等待一小段时间让状态更新
...
def create_iot_query_function(device_name, prop_name, prop_info):
    ...
    async def iot_query_function(context, response_success=None, response_failure=None):
        try:
            # 打印响应参数
            logger.bind(tag=TAG).info(f"查询函数接收到的响应参数: success='{response_success}', failure='{response_failure}'")

            value = await get_iot_status(context, device_name, prop_name)
...
async def handleIotDescriptors(context, descriptors):
    if not context.session_runtime_config.session_function_config.use_function_call_mode:
        return
    wait_max_time = 5
    while context.function_intent_manager is None or not context.function_intent_manager.finish_init:
        await asyncio.sleep(1)
        wait_max_time -= 1
        if wait_max_time <= 0:
            logger.bind(tag=TAG).error("连接对象没有function_processor")
            return
    """处理物联网描述"""
    ...
    for descriptor in descriptors:
        ...
        if context.session_runtime_config.session_function_config.use_function_call_mode:
            ...
            # 在连接级注册设备函数
            if hasattr(context, "function_processor"):
                for func_name in device_functions:
                    context.function_intent_manager.function_registry.register_function(func_name)
                    logger.bind(tag=TAG).info(f"注册IOT函数到function handler: {func_name}")
                    functions_changed = True

    # 如果注册了新函数，更新function描述列表
    if functions_changed and hasattr(context, "function_processor"):
        context.function_intent_manager.upload_functions_desc()
        func_names = context.function_intent_manager.current_support_functions()
...
async def get_iot_status(context, name, property_name):
    """获取物联网状态"""
    for key, value in context.iot_descriptors.items():
        if key == name:
            for property_item in value.properties:
...
async def set_iot_status(context, name, property_name, value):
    """设置物联网状态"""
    for key, iot_descriptor in context.iot_descriptors.items():
        if key == name:
...
async def send_iot_conn(context, name, method_name, parameters):
    """发送物联网指令"""
    for key, value in context.iot_descriptors.items():
        if key == name:
            # 找到了设备
            for method in value.methods:
                ...
                if method["name"] == method_name:
                    ...
                    await context.websocket.send_message(iot_command)
                    return
