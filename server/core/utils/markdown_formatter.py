...
class MarkdownFormatter:
    """
    封装 Markdown 清理逻辑：直接用 MarkdownManager.clean_markdown(text) 即可
    """
    ...
    def _replace_inline_dollar(m: re.Match) -> str:
        ...
        """
        content = m.group(1)
        if MarkdownFormatter.NORMAL_FORMULA_CHARS.search(content):
            return content
        else:
            return m.group(0)
    ...
    def clean_markdown(text: str) -> str:
        ...
        """
        for regex, replacement in MarkdownFormatter.REGEXES:
