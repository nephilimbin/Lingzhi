...
from plugins.functions.hass_init import initialize_hass_handler
import requests
...
def hass_play_music(conn, entity_id="", media_content_id="random"):
    try:
        # 执行音乐播放命令
        future = asyncio.run_coroutine_threadsafe(handle_hass_play_music(conn, entity_id, media_content_id), conn.loop)
