...
class SlidableController {
  ...
  // 设置当前打开的项目 - 使用内部方法
  void _setCurrentOpenTile(_SlidableDeleteTileState tile) {
    // 如果已经有其他打开的项目，先关闭它
...
class _SlidableDeleteTileState extends State<SlidableDeleteTile> {
  ...
  // 内容与删除按钮之间的间距
  final double _contentButtonSpacing = 12.0;
  // 拖动阈值，超过这个比例会自动打开
  final double _openThreshold = 0.3;

  // 总滑动距离 = 按钮宽度 + 间距
  double get _totalDragExtent => _deleteButtonWidth + _contentButtonSpacing;
        ...
        Positioned.fill(
          child: Container(
            ...
            child: AnimatedOpacity(
              ...
              opacity: (_dragExtent.abs() / _totalDragExtent).clamp(0.0, 1.0),
              child: deleteButton,
            ),
          ),
        ),
        ...
        GestureDetector(
          ...
          onHorizontalDragUpdate: (details) {
            setState(() {
              ...
              // 限制范围
              _dragExtent = _dragExtent.clamp(-_totalDragExtent, 0.0);
            });
          },
          onHorizontalDragEnd: (details) {
            ...
            if (velocity > 300 ||
                _dragExtent > -_totalDragExtent * _openThreshold) {
              _resetPosition();
              SlidableController.instance._currentOpenTile = null;
            }
            // 向左滑动速度快，或者已经接近全开，则打开
            else if (velocity < -300 ||
                _dragExtent.abs() >= _totalDragExtent * _openThreshold) {
              // 打开删除按钮
              setState(() {
                _isOpen = true;
                _dragExtent = -_totalDragExtent;
              });
              // 记录当前打开的项目
              SlidableController.instance._setCurrentOpenTile(this);
              // 振动反馈
