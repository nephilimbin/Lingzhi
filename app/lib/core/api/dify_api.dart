...
import 'package:ai_assistant/core/utils/device_util.dart';
...
class DifyService {
    ...
    try {
      ...
      if (mapJson != null && mapJson.isNotEmpty) {
        ...
        loadedMap.forEach((key, value) {
          _sessionConversationMap[key.toString()] = value.toString();
        });

        logI('已从本地存储加载 ${_sessionConversationMap.length} 个会话ID映射');

        // 打印所有已加载的会话ID，方便调试
        _sessionConversationMap.forEach((sessionId, conversationId) {
          logI('已加载会话映射: $sessionId => $conversationId');
        });
      } else {
        logI('没有找到存储的会话ID映射');
      }
  ...
  Future<void> _saveConversationMap() async {
    try {
      ...
      logI('已保存 ${_sessionConversationMap.length} 个会话ID映射到本地存储');
    } catch (e) {
      logE('保存会话ID映射失败: $e');
    }
  }
  ...
  Future<String> _getDeviceUserId() async {
    if (_deviceUserId == null) {
      ...
      logI('使用设备ID作为用户标识 = $_deviceUserId');
    }
  ...
  Future<String?> _getConversationId(String sessionId) async {
    ...
    if (_sessionConversationMap.containsKey(sessionId) &&
      ...
      logI('使用现有的会话ID = ${_sessionConversationMap[sessionId]} 用于会话 $sessionId');
      return _sessionConversationMap[sessionId]!;
    }

    // 对于首次聊天，返回null让API自己创建conversation_id
    logI('未找到会话 $sessionId 的ID，将创建新会话');
    return null;
  }
  ...
  Future<String> sendMessage(
    ...
    try {
      ...
      logI('发送请求到 $requestUrl');
      logI('API Key = ${apiKey.substring(0, math.min(5, apiKey.length))}...');
      logI('会话 ID = $sessionId');
      logI('Conversation ID = $conversationId');
      logI('用户 ID = $_deviceUserId');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['conversation_id'] != null) {
          ...
          logI('保存服务器返回的会话ID = $conversationId');
          await _saveConversationMap(); // 保存更改到本地存储
        }
  ...
  Stream<String> sendMessageStream(
    ...
    try {
      ...
      final requestBody = jsonEncode(requestMap);
      logI('请求体 = $requestBody');
      request.body = requestBody;

      final streamedResponse = await http.Client().send(request);
      logI('响应状态码 = ${streamedResponse.statusCode}');
            ...
            try {
              ...
              if (event['conversation_id'] != null) {
                ...
                if (conversationId != null &&
                  ...
                  logI('警告：服务器返回的会话ID与之前存储的不同');
                  logI('旧ID = $conversationId');
                  logI('新ID = ${event['conversation_id']}');
                }
  ...
  Future<Map<String, dynamic>> uploadFile(
    ...
    try {
      ...
      logI('文件上传响应状态码 = ${response.statusCode}');
